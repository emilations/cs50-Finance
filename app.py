...
def index():
    ...
    balance_share = db.execute(
        "SELECT username_id, symbol, share_name, SUM (quantity) as sum_quantity, price_share, quantity, quantity*price_share AS total_sum FROM transactions WHERE username_id = ? GROUP BY username_id, symbol", session.get("user_id"))
    total_sum = 0
    ...
    balance_cash = db.execute("SELECT cash FROM users WHERE id = ?", session.get("user_id"))
